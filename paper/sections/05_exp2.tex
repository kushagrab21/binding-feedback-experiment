...
\emph{Definition:} Prediction 2 claims a monotone relation between $\Delta$ and capability rank. The Spearman rank correlation~\cite{spearman1904} measures exactly this: it is +1 when $\Delta$ only rises as rank weakens, $-$1 when $\Delta$ only falls, and near 0 when there is no consistent direction. The test is one-sided in the registered direction, and the p-value is exact, computed by comparing the observed correlation against all 5,040 possible orderings of 7 ranks.
